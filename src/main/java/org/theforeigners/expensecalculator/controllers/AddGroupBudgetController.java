...
public class AddGroupBudgetController implements CallbackHandler {
    ...
    private void handleSave() {
        ...
        try {
            ...
            String budgetQuery = "INSERT INTO BUDGET (user_id, group_id, month, year, created_at) VALUES (?, ?, ?, ?, NOW())";
            try (PreparedStatement budgetPs = connection.prepareStatement(budgetQuery,
                    Statement.RETURN_GENERATED_KEYS)) {
                budgetPs.setInt(1, userId);
                budgetPs.setInt(2, groupId);
                budgetPs.setInt(3, month);
                budgetPs.setInt(4, LocalDate.now().getYear());
